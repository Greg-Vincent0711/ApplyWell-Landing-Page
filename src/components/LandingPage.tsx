...
import { Briefcase, ShieldCheck, Zap, CheckCircle2, AlertCircle, ArrowRight, Italic } from 'lucide-react';
      ...
      <main className="max-w-3xl mx-auto px-6 pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <div className="space-y-8 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          <p className="text-xl md:text-2xl text-gray-500 font-medium max-w-lg mx-auto leading-relaxed text-center">
            Built by software developers who hates mass-applying.
            LLMs can speed up job applications — but they shouldn't turn your resume into a template.
            We're building a service that fits neatly in between.
          </p>
        </div>
        ...
        <div className="w-full max-w-md mt-16 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-150">
          {status === 'success' ? <div className="bg-green-50 border border-green-200 rounded-xl p-8 flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
            ...
            </div> : <form onSubmit={handleSubmit} className="relative group">
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-grow">
                  <input type="email" value={email} onChange={e => {
                setEmail(e.target.value);
                if (status === 'error') setStatus('idle');
              }} placeholder="janedoe@yahoo.com" className={`w-full px-6 py-4 bg-white border-2 rounded-xl text-lg outline-none transition-all duration-200 placeholder:text-gray-400
                  ...
                  {status === 'error' && <div className="absolute -bottom-6 left-0 flex items-center text-xs text-red-500 font-medium animate-in slide-in-from-top-1">
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-12 w-full animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
          <BenefitItem icon={<Briefcase className="text-gray-700" size={24} />} title="Automate your Applications Effectively" description="Remove the time-wasting, soul-crushing busy work portion that is mass-appyling for jobs." />
          <BenefitItem icon={<ShieldCheck className="text-gray-700" size={24} />} title="Maintain Control...and Quality" description="Use AI as it was meant to be used - as a tool, not a replacement for your experience. You are always in the loop when you apply with Resumaid." />
          <BenefitItem icon={<Zap className="text-gray-700" size={24} />} title="Apply to Roles that suit you, Skip ones that Don't." description="Recieve analysis before you apply - know on the spot if what you want to apply for may be a waste of your time." />
        </div>
      </main>
    </div>;
}
